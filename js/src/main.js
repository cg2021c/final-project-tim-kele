    ...
    for (var j = 1; j < tiers - 2; j++) {
        ...
        for (var i = 0; i < sides; i++) {
            ...
            if (j % 2 !== 0) {
                ...
                nextVertexVector =
                    sphereGeometry.vertices[i + vertexIndex + 1].clone();
                if (i == sides - 1) {
...
function addCharacter() {
    ...
    sphereGeometry.rotateX(Math.PI / 6);
    sphereGeometry.rotateY(Math.PI / 2);

    jumping = false;
    character = new THREE.Mesh(sphereGeometry, sphereMaterial);
    character.receiveShadow = true;
    character.castShadow = true;
    scene.add(character);
    character.position.y = characterBaseY;
    character.position.z = 4.8;
    currentLane = middleLane;
...
function update() {
    ...
    character.rotation.x -= characterRollingSpeed;
    if (character.position.y <= characterBaseY) {
        jumping = false;
        bounceValue = Math.random() * 0.04 + 0.005;
    }
    character.position.y += bounceValue;
    character.position.x = THREE.Math.lerp(
